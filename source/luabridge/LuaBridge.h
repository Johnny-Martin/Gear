...
public:
	...
	BEGIN_LUACFUNCTION_DECLARE()
		...
		REGISTER_CFUNCTION(SetGlobal)
		REGISTER_CFUNCTION(GetGlobal)
...
namespace LuaCFunction {
	...
	void Push(lua_State* L, const T& t, Args... args) {
		Push(L, t);
		Push(L, args);
	}
	...
	std::tuple<T> _get(lua_State* L, int index) {
		std::make_tuple<T>(Read<T>(L, index));
	}

	template<typename T, typename... Args>
	std::tuple<T, Args...> _get(lua_State* L, int index) {
		std::tuple<T> head = std::make_tuple<T>(Read<T>(L, index));
		std::tuple_cat(head, _get<Args...>(L, index - 1));
	}
